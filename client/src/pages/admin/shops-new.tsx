...
export default function ShopsPage() {
  ...
  // Filter the shops based on the selected filter
  const filteredShops = shops ? shops.filter((shop: Shop) => {
    ...
    <AdminLayout title="Shops Management">
      ...
      <div className="flex justify-between items-center mb-6">
        <div className="flex items-center space-x-2">
          <Label htmlFor="shop-filter">Filter:</Label>
          <Select
            value={filter}
            onValueChange={(value) => setFilter(value as "active" | "inactive" | "all")}
          >
            <SelectTrigger className="w-[180px]" id="shop-filter">
        ...
        {filteredShops.length > 0 ? (
          ...
          <div className="col-span-full text-center py-8">
            <p className="text-gray-500">
              {filter === "active" && "No active shops found."}
              {filter === "inactive" && "No inactive shops found."}
              {filter === "all" && "No shops found."}
            </p>
          </div>
        )}
      </div>
